...
app.listen(settings.PORT, "0.0.0.0", () => {
    ...
    console.log(`transcodeAuto=${settings.TRANSCODE_AUTO_ENABLED ? 1 : 0} ladder=${settings.TRANSCODE_VARIANTS.map(variant => `${variant.height}p:${variant.videoK}k`).join(",")} originalVariant=${settings.TRANSCODE_INCLUDE_ORIGINAL_VARIANT ? 1 : 0} maxSessions=${settings.TRANSCODE_MAX_SESSIONS}`);
