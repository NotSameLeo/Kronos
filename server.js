...
function spawnIngest(ingest) {
    ...
    // Clear leftovers so ffmpeg starts a fresh, monotonic sequence.
    try {
    ...
    const proc = spawn(FFMPEG_BIN, ffmpegArgs(ingest.url, dir), { stdio: ["ignore", "ignore", "pipe"] });
    ingest.proc = proc;
    ingest.pid = proc.pid;
